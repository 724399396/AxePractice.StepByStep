    ...
    public class RestrictedUacAttribute : ActionFilterAttribute
    {
            ...
            CancellationToken token)
        {
            ...
            var userId = context.ActionContext.ActionArguments.Where(kv => kv.Key == userIdArgumentName)
                .Select(kv => (long) kv.Value).ToList();
            if (!userId.Any())
            {
            ...
            dynamic result = JsonConvert.DeserializeObject(await responseContent.ReadAsStringAsync());

            RestrictedUacContractService restrictedUacContractService =
                (RestrictedUacContractService)context.Request.GetDependencyScope()
                .GetService(typeof(RestrictedUacContractService));
            if (restrictedUacContractService.RemoveRestrictedInfo(userId.First(), result))
            {
